...
# Manually corrected a line end for 1605.53.10 that ran on from the previous line in
# the text file

tariff$tariff01 <- gsub("\t \t", "\t\t", tariff$tariff01)
tariff$tariff01 <- gsub("\t \t", "\t\t", tariff$tariff01)
#### Trial this line
tariff$tariff01 <- gsub("\t\t", "", tariff$tariff01) #seems ok
####
tariff$tariff01 <- gsub(" \t", "\t", tariff$tariff01)
...
tariff$tariff01 <- gsub("�", "", tariff$tariff01)
tariff$tariff01 <- gsub("�", "", tariff$tariff01)
tariff$tariff01 <- gsub("- ", "- ", tariff$tariff01) #3808.59.10
tariff$tariff01 <- gsub("- -", "- -", tariff$tariff01) #3808.91.00
tariff$tariff01 <- gsub("\\.\\.$", "\\.\\.", tariff$tariff01) #3824.99.19
...
check <- tariff %>% 
  ...
  filter(branch_hyphen_level==0 & branch_dots_level==0)
